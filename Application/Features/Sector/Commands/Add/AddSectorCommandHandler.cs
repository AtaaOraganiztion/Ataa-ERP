...
public class AddSectorCommandHandler(IMapper mapper, IRepository<Domain.Models.Sector> repository) : ICommandHandler<AddSectorCommand,Ulid>
{
    public async Task<Result<Ulid>> Handle(AddSectorCommand request, CancellationToken cancellationToken)
    {
        var sector = mapper.Map<Domain.Models.Sector>(request);
